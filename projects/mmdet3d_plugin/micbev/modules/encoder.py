...
class BEVFormerEncoder(TransformerLayerSequence):
    ...
    def forward(self,
        ...
        for img_meta in kwargs['img_metas']:
            ...
            real_N = int(img_meta.get('num_real_views', actual_N))
            real_N = max(0, min(real_N, actual_N))
            
            # Create camera validity mask BEFORE padding
            cam_validity_mask = torch.arange(
                actual_N, device=cam_positions_batch.device) < real_N
        ...
        if self.with_gat:
            ...
            # Match BEV query/reference flattening order: y-major rows, then x columns.
            ...
            raise RuntimeError(
                "BEVFormerEncoder requires with_gat=True to compute cam_bev_weights; "
                "equal camera-weight fallback has been disabled."
            )
        # ---------------------------------------------------------------------------
