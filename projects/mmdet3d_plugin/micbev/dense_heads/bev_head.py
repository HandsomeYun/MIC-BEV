...
import mmcv
import cv2 as cv
from projects.mmdet3d_plugin.micbev.modules import PerceptionTransformerBEVEncoder
...
class BEVHead(BaseModule):
    def __init__(self, 
        ...
        # --- build the new BEV-segmentation head, if provided ---
    ...
    def forward(self, mlvl_feats, img_metas, prev_bev=None,  only_bev=False):
        ...
        # --- run semantic head in training & inference ---
        if self.bev_seg_head is not None:
            seg_outs = self.bev_seg_head(bev_feature, gt_seg=img_metas[0].get('bev_seg_label'))
            # merge losses & outputs
            if self.training:
                ret.update({k: v for k, v in seg_outs.items() if k.startswith('loss')})
            else:
                ret['pred_semantic'] = seg_outs['pred_semantic']
    ...
    def loss(self,
        ...
        losses = self.pts_bbox_head_3d.loss(gt_bboxes_list, gt_labels_list, ret['pred'], gt_bboxes_ignore=gt_bboxes_ignore, img_metas=img_metas)
        
        # add semantic loss if present
        if self.bev_seg_head is not None and 'loss_bev_seg' in ret:
            losses['loss_bev_seg'] = ret['loss_bev_seg']
        return losses
