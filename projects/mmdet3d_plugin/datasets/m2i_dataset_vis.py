...
class M2IDataset(NuScenesDataset):
    ...
    def evaluate(self,
        ...
        print_tp_metrics = ['trans_err', 'scale_err', 'orient_err']
        print('mAP: %.4f' % metrics_summary['mean_ap'])
        for tp_name in print_tp_metrics:
        ...
        for name in self.eval_cfg['class_names']:
            ...
            for k in print_tp_metrics:
                v = metrics_summary['label_tp_errors'][name][k]
                val = float('{:.4f}'.format(v))
                detail[f'{metric_prefix}/{name}_{k}'] = val
            for k in print_tp_metrics:
                v = metrics_summary['tp_errors'][k]
                val = float('{:.4f}'.format(v))
        ...
        dump_dir = osp.join(osp.abspath(osp.curdir), "data_dumping")
        mmcv.mkdir_or_exist(dump_dir)
        dump_path = osp.join(dump_dir, "bevformer_bbox_ap.npy")
        np.save(dump_path, per_sample_ap)
        print(f"Saved at {dump_path}")
