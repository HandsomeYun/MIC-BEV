...
from nuscenes.eval.common.loaders import load_prediction, _get_box_class_field
import tqdm
...
def load_gt(nusc: NuScenes, eval_split: str, box_cls, verbose: bool = False):
    ...
    if eval_split == 'val':
        # custom_splits_dir = Path("/media/handsomeyun/LidarTran2/data/test/v1.0-trainval/splits") # Adjust path
        custom_splits_dir = Path(nusc.dataroot) / nusc.version / 'splits'
        val_json_path = custom_splits_dir / "val.json"
        if val_json_path.exists():
            with open(val_json_path, 'r') as f:
                sample_tokens = json.load(f)
        else:
            raise FileNotFoundError(f"Could not find {val_json_path}")
    else: 
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        # print(f"Processing sample_token: {sample_token}")  # Debug statement

        sample = nusc.get('sample', sample_token)
        sample_annotation_tokens = sample['anns']
        # print(f"Number of annotations for this sample: {len(sample_annotation_tokens)}") # Debug statement
    ...
    if verbose:
        ...
        print(f"Total number of ground truth boxes loaded: {len(all_annotations.boxes)}") # Debug statement
    ...
    for sample_token in tqdm.tqdm(sample_tokens, leave=verbose):
        ...
        for ann_token in sample['anns']:
            ...
            # print(f"\nAnnotation {ann_token}:")
            # print(f"Category: {ann['category_name']}")
            # print(f"Translation: {ann['translation']}")
            # print(f"Size: {ann['size']}")
            # print(f"rotation: {ann['rotation']}")
            
            # Check if converted to valid detection name
            det_name = ann['category_name']
            # print(f"Detection name: {det_name}")
            
            if det_name is None:
                print("!! Will be filtered (no detection name mapping)")
...
def filter_eval_boxes(nusc: NuScenes,
    ...
    # Debug: Print max distance thresholds
    print(f"\n[DEBUG] Maximum evaluation distances per class: {max_dist}")
    ...
    dist_rejected = 0
    point_rejected = 0
    bike_rack_rejected = 0
    
    for ind, sample_token in enumerate(eval_boxes.sample_tokens):
        if verbose and ind % 50 == 0:
            print(f"\nProcessing sample {ind+1}/{len(eval_boxes.sample_tokens)}")
        ...
        for box in eval_boxes[sample_token]:
            class_name = box.__getattribute__(class_field)
            if class_name not in max_dist:
                print(f"[WARNING] Box class '{class_name}' not in max_dist config, will be filtered")
                dist_rejected += 1
                continue
            if box.ego_dist >= max_dist[class_name]:
                # if verbose:
                #     print(f"[DEBUG] Filtered box {box.token} (class: {class_name}) - Distance {box.ego_dist:.1f}m > max {max_dist[class_name]}m")
                #     print(f"[DEBUG] Box object: {box}")  # Print the entire box object
                dist_rejected += 1
                continue
            filtered_by_dist.append(box)
        
        eval_boxes.boxes[sample_token] = filtered_by_dist
        dist_filter += len(eval_boxes[sample_token])
        
        if verbose and len(eval_boxes[sample_token]) == 0:
            print(f"[DEBUG] All boxes filtered by distance in sample {sample_token}")
        ...
        for box in eval_boxes[sample_token]:
            if box.num_pts == 0:
                if verbose:
                    print(f"[DEBUG] Filtered box {box.token} - Has 0 points")
                point_rejected += 1
                continue
            filtered_by_points.append(box)
            
        eval_boxes.boxes[sample_token] = filtered_by_points
        point_filter += len(eval_boxes[sample_token])
        
        if verbose and len(eval_boxes[sample_token]) == 0:
            print(f"[DEBUG] All boxes filtered by points in sample {sample_token}")
        ...
        for box in eval_boxes[sample_token]:
            if box.__getattribute__(class_field) in ['bicycle', 'motorcycle']:
                ...
                if in_a_bikerack:
                    if verbose:
                        print(f"[DEBUG] Filtered bike/motorcycle {box.token} - Inside bike rack")
                    bike_rack_rejected += 1
                    continue
            filtered_by_bikerack.append(box)
            
        eval_boxes.boxes[sample_token] = filtered_by_bikerack
        bike_rack_filter += len(eval_boxes.boxes[sample_token])

    # Summary statistics
    print("\n[FILTERING SUMMARY]")
    print(f"=> Original number of boxes: {total}")
    print(f"=> After distance filtering: {dist_filter} (rejected {dist_rejected})")
    print(f"=> After point filtering: {point_filter} (rejected {point_rejected})")
    print(f"=> After bike rack filtering: {bike_rack_filter} (rejected {bike_rack_rejected})")
    print(f"=> Final boxes remaining: {sum(len(boxes) for boxes in eval_boxes.boxes.values())}")
...
class NuScenesEval_custom(NuScenesEval):
    ...
    def __init__(self,
        ...
        print("\n=== DEBUG: Checking GT boxes immediately after loading ===")
        self._print_box_stats(self.gt_boxes, "GT boxes after load_gt()")
        ...
        print("\n=== DEBUG: Checking GT boxes immediately after loading ===")
        self._print_box_stats(self.gt_boxes, "GT boxes after add_center_dist")
        ...
        self._print_box_stats(self.gt_boxes, "GT boxes after filter_eval_boxes()")
        ...
        self._print_box_stats(self.gt_boxes, "GT boxes after filter_eval_boxes_by_overlap()")
    ...
    def _print_box_stats(self, eval_boxes, description):
        """Helper method to print box statistics for debugging."""
        print(f"\n{description} statistics:")
        print(f"- Number of samples: {len(eval_boxes.sample_tokens)}")
        
        total_boxes = 0
        invalid_boxes = 0
        sample_with_invalid = 0
        
        for sample_token in eval_boxes.sample_tokens:
            boxes = eval_boxes[sample_token]
            ...
            for box in boxes:
                if box is None:
                    invalid_boxes += 1
                    sample_invalid += 1
                    print(f"  !! Found None box in sample {sample_token}")
                    continue
                    
                if not hasattr(box, 'translation') or box.translation is None:
                    invalid_boxes += 1
                    sample_invalid += 1
                    print(f"  !! Box with invalid translation in sample {sample_token}")
                    continue
                    
                if not hasattr(box, 'size') or box.size is None:
                    invalid_boxes += 1
                    sample_invalid += 1
                    print(f"  !! Box with invalid size in sample {sample_token}")
                    continue
                    
                if not hasattr(box, 'rotation') or box.rotation is None:
                    invalid_boxes += 1
                    sample_invalid += 1
                    print(f"  !! Box with invalid rotation in sample {sample_token}")
                    continue
                    
                if not hasattr(box, 'detection_name') or box.detection_name is None:
                    invalid_boxes += 1
                    sample_invalid += 1
                    print(f"  !! Box with invalid detection_name in sample {sample_token}")
                    continue
            
            if sample_invalid > 0:
                sample_with_invalid += 1
                print(f"  Sample {sample_token} has {sample_invalid} invalid boxes out of {len(boxes)}")
        
        print(f"\nSummary for {description}:")
        print(f"- Total boxes: {total_boxes}")
        print(f"- Invalid boxes: {invalid_boxes}")
        print(f"- Samples with invalid boxes: {sample_with_invalid}")
        if invalid_boxes > 0:
            print("WARNING: Found invalid boxes in the data!")
...
if __name__ == "__main__":
    ...
    nusc_eval.print_gt_bboxes(num_samples=5)
