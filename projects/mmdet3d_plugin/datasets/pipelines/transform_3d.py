...
class DebugPassThrough(object):
    def __call__(self, data):
        print("[DEBUG] DebugPassThrough called")
        if data is None:
            print("[DEBUG] DebugPassThrough: Input data is None!")
            return None  # Important: Propagate the None!

        print("[DEBUG] DebugPassThrough: Data keys and types:")
        for key, value in data.items():
            print(f"  - {key}: {type(value)}")
            if key in ['gt_bboxes_3d', 'gt_labels_3d']:
                if value is not None:
                    try:
                        print(f"    - Shape: {value.shape}") # For numpy arrays
                    except AttributeError:
                        try:
                            print(f"    - Shape: {value.data.shape}") # For DataContainers
                        except AttributeError:
                            try:
                                print(f"    - Tensor shape: {value.tensor.shape}") # For boxes
                            except AttributeError:
                                pass # If it doesn't have a shape attribute
        return data
...
class PhotoMetricDistortionMultiViewImage:
    ...
    def __init__(self,
        ...
        print("PhotoMetricDistortionMultiViewImage", flush=True)
...
class CustomCollect3D(object):
    ...
    def __init__(self,
                 ...
                 meta_keys=('filename', 'ori_shape', 'img_shape', 'lidar2img','lidar2cam',
                            ...
                            'can_bus', 'bev_seg_label', 'bev_obj_label',
                            'mask_multiview_info', 'num_real_views'
                            )):
        self.keys = keys
